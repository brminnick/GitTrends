namespace GitTrends.Mobile.Shared
{
    static class TrendsPageAutomationIds
    {
        public const string ActivityIndicator = nameof(ActivityIndicator);

        public const string TrendsChart = nameof(TrendsChart);
        public const string TrendsChartLegend = nameof(TrendsChartLegend);
        public const string TrendsChartPrimaryAxis = nameof(TrendsChartPrimaryAxis);
        public const string TrendsChartSecondaryAxis = nameof(TrendsChartSecondaryAxis);

        public const string TotalViewsSeries = nameof(TotalViewsSeries);
        public const string TotalUniqueViewsSeries = nameof(TotalUniqueViewsSeries);
        public const string TotalClonesSeries = nameof(TotalClonesSeries);
        public const string TotalUniqueClonesSeries = nameof(TotalUniqueClonesSeries);
    }
}
