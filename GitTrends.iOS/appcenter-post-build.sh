...
if [ "$APPCENTER_XAMARIN_CONFIGURATION" == "Release" ];then
    ...
    appcenter test run uitest --app "CDA-Global-Beta/GitTrends-iOS-Debug" --devices "CDA-Global-Beta/app-store-screenshots" --app-path $IPAFile --test-series "app-store-screenshots" --locale "en_US" --build-dir $UITestBuildDir --dsym-dir $DSYMFile --uitest-tools-dir $TestCloudExeDirectory --async
fi
